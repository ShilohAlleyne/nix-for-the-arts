...
// Title
#set document(
  title: [Nix and Open Source Software for the Arts]
)
...
#columns(2, gutter: 8pt)[
  ...
  By building software as a nix package, nix also becomes a means for software distribution. The command `nix run` allows for the allows for the execution of applications or binaries provided by Nix packages or flakes. Making it a streamlined way to run software without installing it system-wide. The command: ```bash nix run blender-bin```. Runs the default app from the `blender-bin` flake. To use nix run the flake must evaluate to an app or regular nix derivation. If it evaluates to an app, then `nix run` executes the program specified by the app definition. 
...
#columns(2, gutter: 8pt)[
  ...
  #framed_terms[
    #terms.item[Lockfile][A type of file generated typically generated by a package manager, it is typically not meant to be edited but is rather a reflection of the resolved dependency tree for a project @gamage2025designspacelockfilespackage . They usually have the file type `.lock`]
  ]
...
#columns(2, gutter: 8pt)[
  ...
  In conclusion, building from source is an excellent way of ensuing software reproducibility and through the use of Nix
